...
6. 발행 ✅
7. **카페 공유 덧붙임 글**(있으면) 📋 복사 → 카페에 블로그 링크와 함께 올리기
   - 블로그 본문 그대로 공유하면 유사문서로 저품질 처리될 수 있어요
   - 이 덧붙임 글(공유 이유·내 생각·질문)을 함께 올리면 점수가 올라갑니다
...
# 4) 카페 공유 덧붙임 글
# ---------------------------------------------------------------------------
cafe_note = data.get("cafe_note", "")
if cafe_note:
    st.header("4. 카페 공유 덧붙임 글")
    st.caption(
        "블로그를 카페에 공유할 때 함께 올리는 글이에요. 블로그 본문과 겹치지 않는 "
        "'공유 이유·내 생각·질문'이라 저품질(유사문서) 분류를 피하고 점수를 올려줍니다. "
        f"글자 수: {len(cafe_note):,}자 (500자 내외 권장) · 박스 우측 상단 📋 로 복사하세요."
    )
    st.code(cafe_note, language=None)
    st.divider()


# ---------------------------------------------------------------------------
# 5) 이미지 텍스트 패키지 (Canva 작업 참고용)
# ---------------------------------------------------------------------------
st.header("5. 이미지 텍스트 패키지")
